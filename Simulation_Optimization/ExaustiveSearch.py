...
# Optimization function
def optimize_capacity_and_caregivers_with_reserved_beds(solutions, func):
    """
    Perform brute-force search for optimal capacity and numbers of care givers.
    Returns a result dictionary containing waiting times and penalties for each combination.
    """
    ...
    for num_capacity, num_care_givers, num_reserved_beds in solutions:
        random.seed(random_seed)
        np.random.seed(random_seed)
...
if best_strategy in ["FIFO", "PQ", "DPQ"]:
    ...
    print(f"Additional Capacity: {best_config[0] - original_num_beds}, Additional Number of Care Givers: {best_config[1] - original_num_caregivers}, Additional Number of Reserved Beds: {best_config[2]}")
    print(f"Average Queueing Waiting Time: {optimal_avg_queueing_waiting_time:.2f} minutes")
    print(f"Average Service Waiting Time: {optimal_avg_service_waiting_time:.2f} minutes")
    print(f"ICU Queue Penalty: {optimal_IcuQueue_penalty:.2f}")
    print(f"Care Request Penalty: {optimal_CareRequest_penalty:.2f}")
...
for func_name, results in all_results.items():
    ...
    # If the strategy includes reserved beds, plot a 3D chart
        ...
        norm = mcolors.LogNorm(vmin=max(min(total_penalties), 1e-3), vmax=max(total_penalties))
        min_penalty_legend_added = False
        for i in range(len(total_penalties)):
            if total_penalties[i] == min_penalty and not min_penalty_legend_added:
                    ax.scatter(
                        capacity_list[i],
                        care_givers_list[i],
                        ...
                        s=150,
                        linewidth=2,
                        label='Min Penalty',  # Only add label at the first time
                        zorder=2
                    )
                    min_penalty_legend_added = True  
            elif total_penalties[i] == min_penalty:
                ax.scatter(
                    capacity_list[i],
                    care_givers_list[i],
                    reserved_beds_list[i],
        ...
        ax.text(
            min_penalty_point[0],
            min_penalty_point[1],
            min_penalty_point[2] + 0.5,  # Add an offset to avoid overlapping
            f"{min_penalty:.2f}",
            color='red',
            fontsize=10,
            fontweight='bold',
...
Execution Times for Each Strategy:
DPQ and Reserved Beds Strategy: 2761.25 seconds
PQ and Reserved Beds Strategy: 2404.40 seconds
Reserved Beds Strategy: 1847.96 seconds
FIFO Strategy: 45.62 seconds
PQ Strategy: 46.14 seconds
DPQ Strategy: 45.78 seconds
'''
