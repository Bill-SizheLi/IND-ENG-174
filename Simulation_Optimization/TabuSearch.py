...
def tabu_search(initial_guess, budget, price_bed, price_caregiver, original_num_beds, original_num_caregivers,
    ...
    if current_point[2] in ["FIFO", "PQ", "DPQ"]:
        local_minimizers[current_point[2]].append((capacity, num_caregivers, 0, total_penalty))
    else:
         local_minimizers[current_point[2]].append((capacity, num_caregivers, reserved_beds, total_penalty))
    ...
    while True:
        ...
        local_minimizers[current_minimizer[2]].append((original_num_beds + current_minimizer[0], original_num_caregivers + current_minimizer[1], current_minimizer[3], current_minimum))  # Update to the current point

        optimal_point_current_iter = current_minimizer
        iteration += 1
    return local_minimizers




local_minimizers = tabu_search(initial_guess, budget, price_bed, price_caregiver, original_num_beds, original_num_caregivers)
print(local_minimizers)

def plot_best_points(local_minimizers, strategies):
    """
    ...
    for strategy, values in local_minimizers.items():
        for value in values:  # Iterate through all values for the strategy
            beds, caregivers, reserved_beds, penalty = value
            '''
            x_labels.append(f"{beds}, {caregivers}")  # Combine beds and caregivers for labels
    ...
    ax.set_title("Local Minimizers Visited", pad=20)
...
plot_best_points(local_minimizers, strategies)
