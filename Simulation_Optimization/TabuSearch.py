...
def tabu_search(initial_guess, budget, price_bed, price_caregiver, original_num_beds, original_num_caregivers,
    ...
    if current_point[2] == "FIFO":
        best_results["FIFO"] = (capacity, num_caregivers, 0, total_penalty)
    elif current_point[2] == "PQ":
        best_results["PQ"] = (capacity, num_caregivers, 0, total_penalty)
    elif current_point[2] == "DPQ":
        best_results["DPQ"] = (capacity, num_caregivers, 0, total_penalty)
    elif current_point[2] == "Reserved":
        best_results["Reserved"] = (capacity, num_caregivers, reserved_beds, total_penalty)
    elif current_point[2] == "Reserved+PQ":
        best_results["Reserved+PQ"] = (capacity, num_caregivers, reserved_beds, total_penalty)
    elif current_point[2] == "Reserved+DPQ":
        best_results["Reserved+DPQ"] = (capacity, num_caregivers, reserved_beds, total_penalty)
    ...
    while True:
        ...
        for neighbor_point in neighbor_points:
            if neighbor_point not in tabu_list:
                ...
                if best_results[current_point[2]] is None or total_penalty < best_results[current_point[2]][3]:
                    best_results[current_point[2]] = (original_num_beds + current_point[0], original_num_caregivers + current_point[1], current_point[3], total_penalty)  # Update to the current point
